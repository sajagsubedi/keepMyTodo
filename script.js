...
notesitems+=`
  <div class="lg:flex my-3 min-h-[100px] min-w-[100px] mx-3 rounded lg:items-center w-1/1 bg-gray-800 lg:justify-between">
  <div class="min-w-fit  min-h-[60px] flex-1">
  <h2 class="text-2xl text-white font-bold leading-7  sm:truncate sm:text-3xl sm:tracking-tight"><span class="text-violet-500 mx-2">${index+1}</span>${element[0]}</h2>
  <div class="mt-1 flex flex-col sm:mt-0 sm:flex-row sm:flex-wrap sm:space-x-6">
  <div class="mt-2 flex items-center text-1xl text-gray-400">
  ${element[1]}
  </div> 
  </div>
  </div>
  <span class="sm:ml-3 min-w-fit flex  items-center justify-center ">
  <button onclick="deletei(${index})" type="button" class="inline-flex items-center rounded-md border border-transparent bg-blue-600 px-4 py-1 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
  Delete
  </button>
  </span>
  </div>`
